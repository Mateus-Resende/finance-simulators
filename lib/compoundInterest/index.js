...
compoundInterest.calculate = function (initial, monthly, profitability, period, interestIsMonthly = true) {
  ...
  const total = totalInvestment(initial, monthly, period)
  const accrued = accruedEarnings(initial, monthly, profitability, period)
...
const monthlyInterest = function (interest, interestIsMonthly) {
  const value = interestIsMonthly ? interest : Math.pow(1 + parseFloat(interest), 1 / 12) - 1
