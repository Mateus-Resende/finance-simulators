...
// F = P.(1 + i)^n
const getFutureValue = function (amount, rate, length) {
  return +(amount * (Math.pow(1 + rate, length))).toFixed(2)
}

// F = P.(1+i)^n + M.[(1+i)^n - 1]/i
const getFutureValueMonthlyPayments = function (amount, rate, length, payment) {
  return +(
...
// (1 + r)^(1/12) - 1
const getMonthlyRate = function (annualRate) {
  return +(Math.pow(1 + annualRate, 1 / 12) - 1).toFixed(6)
}

// (1 + r)^12 - 1
const getAnnualRate = function (monthlyRate) {
